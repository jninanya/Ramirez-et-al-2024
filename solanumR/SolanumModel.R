SolanumModel <- function(weather, sowing, hasvest, EDay, plantDensity, CropParams){
...
#if(sum(!(c("date", "tmin", "tmax", "srad", "sunsh") %in% colnames(climate)))>=1)
#  stop()

##### EnvrParams <- c(4,15,28,12,0.5)
...
wmax = CropParams["wmax"]
tm = CropParams["tm"]
te = CropParams["te"]

RUE = CropParams["RUE"]
DMcont = CropParams["DMc"]
...
out <- data.frame(date=climate$date,tt=climate$TT,cc,tmin=climate$tmin,
                  tmax=climate$tmax,N=climate$sunsh,srad=climate$srad,
                  dW,tdm,Tindex,Pindex,TII,ctii,Part,Part_cTII,HI_cTII,
                  dty1,dty2,dty,fty,tdm,check.names=FALSE)
